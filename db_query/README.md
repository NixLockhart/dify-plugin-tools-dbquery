...
Just fill in the connection parameters / 直接填写连接参数即可：
...
The underlying connection URL is `dm+dmPython://<username>:<password>@<host>:<port>/`.

底层连接串为 `dm+dmPython://<用户名>:<密码>@<主机>:<端口>/`。
